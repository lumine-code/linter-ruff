...
module.exports = {
  ...
  activate() {
    ...
    this.disposables.add(
      ...
      atom.config.observe("linter-ruff.hint", (value) => {
  ...
  consumeIdeClient(ideClient) {
    ...
    const subscription = ideClient.onDidChangeAdapters?.(() => {
  ...
  isServedByIdeRuff(editor) {
    ...
    return adapters.some((adapter) => adapter.id === IDE_RUFF_ADAPTER_ID);
  },
  ...
  pythonEditor() {
    ...
    if (!this.grammarScopes.includes(editor.getGrammar().scopeName)) {
      atom.notifications.addWarning("Not a Python file");
      return null;
    }
    return editor;
  },
